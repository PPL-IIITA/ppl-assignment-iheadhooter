...
if __name__ == '__main__': # Execution Begins from Here
	...
	logging.basicConfig(filename='q5Log.txt',level=logging.DEBUG)			
	boysList, girlsList, giftsList, couplesList = [], [], [], []		
	loadData()				 			#calls the function which loads data from csv file to Program
	#print len(giftsList)	

	algo = int(raw_input("press 1 for choosing allottment algorithm same as q1 and press 2 for choosing new allottment algorithm ---->"))
	
	if algo == 2:
		attractiveGirls = sorted(girlsList, key = lambda x : x.attractiveness, reverse = True)
		girlsList.sort(key = lambda x: x.maintainenceBudget, reverse = False)
		boysList.sort(key = lambda x: x.attractiveness, reverse = False)
		...
		while loop:
			...
			try:
				for girl in girlsList:
					if girl.isCommitted == False:
						temp = girl
						break

				for boy in boysList:
					if temp.canDateThisBoy(boy) and boy.canDateThisGirl(temp):
						temp.getCommitted(boy)
						boy.getCommitted(temp)
						couplesList.append(Couple(temp, boy))
						logging.info("%s >>> boy %s and girl %s just got committed" %(str(datetime.now()), boy.name, temp.name))
						loop = 1
						break

				for boy in boysList:
					if boy.isCommitted == False:
						temp = boy
						break

				for girl in attractiveGirls:
					if girl.canDateThisBoy(temp) and boy.canDateThisGirl(girl):
						girl.getCommitted(temp)
						temp.getCommitted(girl)
						couplesList.append(Couple(girl, temp))
						logging.info("%s >>> boy %s and girl %s just got committed" %(str(datetime.now()), temp.name, girl.name))
						break
